...
uint8 UART_RxStringSize(char *ptr_string , uint8 Size)
{
	char ch;
	uint8 len = 0;
	while(1)
	{
		...
		{						     //once enter key is pressed null terminate the string
			...
			if (len<8)
			{
				ptr_string[len]=0xff ; //put 0xff into string and increment the index
				len++;
			}
			break;
		}
...
void UART_TxNumber(uint8 v_numericSystem_u8, uint32 v_number_u32, uint8 v_numOfDigitsToTransmit_u8)
{
    uint8 i=0,a[10];
...
void UART_TxFloatNumber(f32 v_floatNumber_f32)
{
	uint32 v_tempNumber_u32;
	...
	v_tempNumber_u32 = (uint32) v_floatNumber_f32;
...
void UART_TxFloatNumberD(f32 v_floatNumber_f32 , uint8 Digit_Num)
{
	...
	v_floatNumber_f32 = v_floatNumber_f32 - v_tempNumber_u32;
	v_tempNumber_u32 = v_floatNumber_f32 * 1000000;
	UART_TxNumber(C_DECIMAL_U8,v_tempNumber_u32,Digit_Num);
}
#endif
...
void UART_Printf(const char *argList, ...)
{
	...
	char *str;
	char  ch;
	uint8 v_numOfDigitsToTransmit_u8;
#if (Enable_UART_TxFloatNumber==1)
	f32 v_floatNum_f32;
#endif	
...
#endif
void ERROR (uint8 Error_Typ)
{
	switch (Error_Typ)
	{
		case INVALID_USER_OR_PASS :
			UART_Printf("\n\rInvalid User_Name Or Password");
			break;
