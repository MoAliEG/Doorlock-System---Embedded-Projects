	...
	#endif
	{
		...
		#endif
		{
			switch(CfgPtr->PinPort)
			{
				case PORT_A:
				{
					STD_SET_BIT(PORTA,CfgPtr->PinNum);
				}break;
				case PORT_B:
				{
					STD_SET_BIT(PORTB,CfgPtr->PinNum);
				}break;
				case PORT_C:
				{
					STD_SET_BIT(PORTC,CfgPtr->PinNum);
				}break;
				case PORT_D:
				{
					STD_SET_BIT(PORTD,CfgPtr->PinNum);
				}break;
				default:
				{
	...
	#endif
	{
		...
		#endif
		{
			switch(CfgPtr->PinPort)
			{
				case PORT_A:
				{
					STD_CLR_BIT(PORTA,CfgPtr->PinNum);
				}break;
				case PORT_B:
				{
					STD_CLR_BIT(PORTB,CfgPtr->PinNum);
				}break;
				case PORT_C:
				{
					STD_CLR_BIT(PORTC,CfgPtr->PinNum);
				}break;
				case PORT_D:
				{
					STD_CLR_BIT(PORTD,CfgPtr->PinNum);
				}break;
				default:
				{
...
Std_RetTyp Dio_Write_Bit (Dio_ChannelIdTyp ChannelId , uint8 u8Value)
{
	const Dio_ChannelCfgTyp * CfgPtr = NULL;
	Std_RetTyp RetVal = STD_OK;
	boolean u8ValueS ;
	...
	#endif
	{
		...
		#endif
		u8ValueS = STD_GET_BIT(u8Value , CfgPtr->PinNum) ;
		{
			switch(CfgPtr->PinPort)
			{
				case PORT_A:
				{
					if (STD_HIGH == u8ValueS) STD_SET_BIT(PORTA,CfgPtr->PinNum);
					else STD_CLR_BIT(PORTA,CfgPtr->PinNum);
				}break;
				case PORT_B:
				{
					if (STD_HIGH == u8ValueS) STD_SET_BIT(PORTB,CfgPtr->PinNum);
					else STD_CLR_BIT(PORTB,CfgPtr->PinNum);
				}break;
				case PORT_C:
				{
					if (STD_HIGH == u8ValueS) STD_SET_BIT(PORTC,CfgPtr->PinNum);
					else STD_CLR_BIT(PORTC,CfgPtr->PinNum);
				}break;
				case PORT_D:
				{
					if (STD_HIGH == u8ValueS) STD_SET_BIT(PORTD,CfgPtr->PinNum);
					else STD_CLR_BIT(PORTD,CfgPtr->PinNum);
				}break;
				default:
				{
...
void Dio_Write_Reg (volatile uint8* Reg , ePinsTyp Pin_Num, boolean Val)
{
	if (Val != STD_LOW) STD_SET_BIT(*Reg , Pin_Num);
	else STD_CLR_BIT(*Reg , Pin_Num);
